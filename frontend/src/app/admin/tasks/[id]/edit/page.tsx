...
export default function EditTaskPage() {
  ...
  useEffect(() => {
    if (taskId) {
      api.tasks.getById(taskId).then((t) => {
        setTask(t);
        setForm({ name: t.name, category: t.category, siteProject: t.siteProject, deadline: t.deadline.split("T")[0], priority: t.priority, description: t.description });
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    setSaving(true);
    try { await api.tasks.update(taskId, form); router.push("/admin"); }
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Site/Project</label>
